...
//定义传递消息的全局变量
ros::NodeHandle nh;
skeleton_msg::Skeleton twist_msg;
...
)
{
	...
	//ros::NodeHandle nh;
	char *ros_master = "166.111.138.143";
                    ...
                    if (SUCCEEDED(hr) && bTracked)
                    {
                        ...
                        if (SUCCEEDED(hr))
                        {
                            for (int j = 0; j < _countof(joints); ++j)
                            {
								...
								ofile<<joints[j].Position.X<<","<<joints[j].Position.Y<<","<<joints[j].Position.Z<<",";   //数据写入文件

								
								twist_msg.skeleton[j * 3 + 0] = joints[j].Position.X;
								twist_msg.skeleton[j * 3 + 1] = joints[j].Position.Y;
								twist_msg.skeleton[j * 3 + 2] = joints[j].Position.Z;
